...
public class BoardDetailPage extends AppCompatActivity implements View.OnClickListener {
    ...
    Button comment_popup_btn, comment_btn;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        comment_popup_btn = (Button) findViewById(R.id.comment_popup_btn);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.comment_info_ok_btn:
                comment_params.put("WRITE_NAME",comment_name_editText.getText().toString());
                comment_params.put("PASSWD", comment_passwd_edittext.getText().toString()); //확인필요


            case R.id.comment_popup_btn:
                comment_info_layout.setVisibility(View.VISIBLE);

                //commentinfo gone if fill out name and passwd
                if (comment_name_editText.getText() != null && comment_passwd_edittext.getText() != null) {
...
//                if()
            ...
            case R.id.comment_btn:
                ...
                //커맨드 데이터 보내기
                comment_params = new RequestParams();
                SharedPreferences sp = getSharedPreferences("GCM", MODE_PRIVATE);

                comment_params.put("CALL_BOARD_NO", board_no);
                comment_params.put("CONTENTS", comment_editText.getText().toString());
                comment_params.put("REG_ID", sp.getString("reg-id", ""));

                LoopjConnection.getInstance().addComment(comment_params);
//todo test 필요함...
                finish();
